...
const Profile: React.FC<ProfileProps> = ({ projects, contracts, currentUser, conversations, onAddMicroContract, onUpdateContract, onSendMessage, onLogout }) => {
  ...
  return (
    <div className="pt-24 md:pt-32 pb-32 md:pb-48 px-4 md:px-12 bg-black min-h-screen selection:bg-[#007AFF]">
      <Header userId={currentUser?.id} onLogout={onLogout} />
      
      {/* Transactional Modals - Adjusted for Mobile */}
      {bookingContractId && (
          <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-4 md:p-6 backdrop-blur-md animate-in fade-in duration-300">
              <div className="max-w-xl w-full border border-white/10 p-8 md:p-16 space-y-8 md:space-y-12 bg-black shadow-[0_0_150px_rgba(0,122,255,0.15)] overflow-y-auto max-h-[90vh]">
                  <div className="space-y-4">
                    <h2 className="syne text-4xl md:text-6xl tracking-tighter">Authorize Escrow</h2>
                    <p className="opacity-40 text-sm md:text-lg leading-relaxed font-medium">Commit $ {contracts.find(c => c.id === bookingContractId)?.price} to the protocol vault.</p>
                  </div>
                  <div className="bg-white/5 p-6 md:p-10 space-y-6">
                      ...
                      <div className="flex justify-between uppercase text-[8px] md:text-[10px] font-black tracking-[0.2em] md:tracking-[0.4em] text-[#007AFF]">
                          <span>SIGNAL NETWORK FEE</span>
                          <span>WAVED (BETA)</span>
                      </div>
                  </div>
                  <div className="flex flex-col gap-4 md:gap-6">
                    <Button fullWidth size="lg" onClick={confirmBooking} className="!bg-[#007AFF] !text-white border-none py-4 md:py-8">Commit Funds</Button>
                    <button onClick={() => setBookingContractId(null)} className="text-[9px] uppercase font-black opacity-30 hover:opacity-100 transition-opacity tracking-[0.3em]">Abort Protocol</button>
                  </div>
              </div>
          </div>
      )}
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <section className="mb-20 md:mb-48 grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-16 items-start pb-16 md:pb-24 border-b border-white/5">
          <div className="lg:col-span-8 space-y-8 md:space-y-12">
            ...
            <h1 className="syne text-[14vw] sm:text-[10vw] md:text-[8vw] tracking-tighter leading-[0.85] uppercase">
              {user?.name}
            </h1>
            <div className="flex gap-8 md:gap-12 opacity-30 uppercase text-[9px] md:text-[10px] font-black tracking-[0.3em] md:tracking-[0.5em] pt-4 md:pt-8">
                <div className="space-y-1">
                  <span className="block opacity-50">EVIDENCE</span>
                  <span className="text-white text-base md:text-lg tabular-nums">{userProjects.length}</span>
                </div>
                <div className="h-10 md:h-12 w-px bg-white/20" />
                <div className="space-y-1">
                  <span className="block opacity-50">UTILITY</span>
                  <span className="text-white text-base md:text-lg tabular-nums">{user?.sessionsCompleted}</span>
                </div>
            </div>
          </div>
          
          <aside className="lg:col-span-4">
             <GlassCard hover={false} className="!p-8 md:!p-16 space-y-8 md:space-y-12 !bg-[#007AFF]/5 border-[#007AFF]/20 text-center relative overflow-hidden group">
                ...
                <div className="flex justify-center mb-4 md:mb-8 relative z-10">
                   <SignalGauge score={signalScore} size={window.innerWidth < 640 ? 'md' : 'lg'} />
                </div>
                ...
                {!isOwnProfile && (
                  <Button variant="primary" size="lg" fullWidth className="!py-6 md:!py-8 !bg-[#007AFF] !text-white border-none hover:shadow-[0_15px_40px_rgba(0,122,255,0.4)] relative z-10 !text-sm">
                    Secure Expertise
                  </Button>
                )}
        ...
        {/* Dashboard Content - Optimized for Mobile */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 md:gap-24">
          ...
          <div className="lg:col-span-8 space-y-24 md:space-y-48">
              ...
              <div className="space-y-6 md:space-y-12">
                {userProjects.map((project) => (
                  <Link to={`/project/${project.id}`} key={project.id} className="block group">
                    <article className="bg-white/[0.02] border border-white/5 p-6 md:p-12 hover:bg-white/[0.04] hover:border-white/20 transition-all duration-700">
                ...
                {!userProjects.length && <div className="py-16 text-center opacity-10 italic serif text-3xl md:text-4xl">No proofs discovered.</div>}
              </div>
            </section>

            {/* Communication Terminal - Optimized for Mobile */}
            <section className="pt-16 md:pt-24 border-t border-white/5 space-y-10 md:space-y-16">
                ...
                {hasCompletedSessionWithUser || isOwnProfile ? (
                    <GlassCard hover={false} className="!p-0 border-white/10 overflow-hidden !bg-black">
                        <div className="p-4 md:p-10 bg-white/5 border-b border-white/10">
                        ...
                        <div className="p-4 md:p-10 space-y-6 md:space-y-10 min-h-[300px] md:min-h-[400px] max-h-[500px] md:max-h-[600px] overflow-y-auto scrollbar-hide flex flex-col justify-end">
                        ...
                        {!isOwnProfile && (
                          <div className="p-4 md:p-10 bg-white/5 border-t border-white/10 flex gap-3 md:gap-6">
                              <input 
                                  className="flex-1 bg-black border border-white/10 p-4 md:p-6 outline-none text-base md:text-lg focus:border-[#007AFF] transition-all font-medium"
                                  placeholder="Secure msg..."
                                  value={chatText}
                                  onChange={e => setChatText(e.target.value)}
                                  onKeyPress={e => e.key === 'Enter' && handleSendChat()}
                              />
                              <Button size="md" onClick={handleSendChat} className="!px-6 md:!px-12">Transmit</Button>
                          </div>
                        )}
                    </GlassCard>
                ) : (
                    <div className="border border-white/5 p-12 md:p-32 text-center bg-white/[0.01] grayscale">
                        ...
                        <Button variant="secondary" size="md" disabled className="!px-8 md:!px-12 opacity-50">Locked</Button>
                    </div>
                )}
            </section>
          </div>

          {/* Right Sidebar Exchanges - Optimized for Mobile */}
          <aside className="lg:col-span-4 space-y-12 md:space-y-24">
            <section className="border border-white/10 p-6 md:p-10 bg-black/80 backdrop-blur-xl space-y-8 md:space-y-12 sticky top-24 md:top-32">
              <div className="flex justify-between items-baseline">
                <h2 className="syne text-2xl md:text-3xl uppercase tracking-tighter">Exchanges</h2>
                {isOwnProfile && (
                  <button onClick={() => setIsAddingContract(true)} className="text-[8px] md:text-[10px] font-black uppercase border-b-2 border-[#007AFF] text-[#007AFF] pb-1 tracking-widest">+ NEW GIG</button>
                )}
              </div>

              {isAddingContract && (
                <div className="bg-[#007AFF]/5 border border-[#007AFF]/20 p-6 md:p-10 space-y-6 md:space-y-8 animate-in slide-in-from-top duration-500">
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea placeholder="Scope..." className="w-full bg-white/5 border border-white/10 p-4 text-xs outline-none h-24 resize-none leading-relaxed" value={newContract.description} onChange={e => setNewContract({...newContract, description: e.target.value})} />
                  </div>
                  <div className="flex flex-col gap-4">
                    <select className="bg-black border border-white/10 p-3 text-[10px] font-black uppercase tracking-widest outline-none" value={newContract.price} onChange={e => setNewContract({...newContract, price: Number(e.target.value)})}>
                        <option value={10}>Tier I ($10)</option>
                        <option value={25}>Tier II ($25)</option>
                        <option value={50}>Tier III ($50)</option>
                    </select>
                    <Button size="md" onClick={handleAddContract} fullWidth className="!py-4">Publish</Button>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-6 md:space-y-8">
                {userContracts.map(mc => {
                    const isBuyer = mc.buyerId === currentUser?.id;
                    const isSeller = mc.userId === currentUser?.id;

                    return (
                        <div key={mc.id} className={`p-6 md:p-10 border transition-all duration-700 ${mc.status !== ContractStatus.AVAILABLE ? 'border-[#007AFF] bg-[#007AFF]/5' : 'border-white/10 hover:border-white/40'}`}>
                            ...
                            {mc.status === ContractStatus.AVAILABLE ? (
                                !isOwnProfile ? (
                                    <Button variant="secondary" size="md" fullWidth className="!py-3" onClick={() => setBookingContractId(mc.id)}>Initiate Protocol</Button>
                                ) : (
